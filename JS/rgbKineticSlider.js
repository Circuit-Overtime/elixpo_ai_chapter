    ...
    window.rgbKineticSlider = function(options) {
       ...
       function slideTransition(next) {
        ...
        function cursorInteractive() {
            ...
            document.getElementById("rgbKineticSlider").addEventListener("mousemove", onPointerMove);
            document.getElementById("rgbKineticSlider").addEventListener("touchmove", onTouchMove);
        ...
        if (options.nav == true) {
            ...
            let nav = document.querySelectorAll('.next');
            ...
            for (let i = 0; i < nav.length; i++) {
                ...
                navItem.onclick = function(event) {
                    console.log(slideImages[currentIndex]);
                    ...
                    if (this.getAttribute('data-nav') === 'next') {
                        // If the current index is the last one, wrap to the first image
                        if (currentIndex >= options.slideImages.length - 1) {
                            slideTransition(0);
                        } else {
                            slideTransition(currentIndex + 1);
                        }
                    ...
                    // Handle "previous" navigation
                        ...
                        // If the current index is the first one, wrap to the last image
                        if (currentIndex <= 0) {
                            slideTransition(options.slideImages.length - 1);
                        } else {
                            slideTransition(currentIndex - 1);
